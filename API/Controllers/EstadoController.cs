...
namespace API.Controllers
{
    public class EstadoController : BaseController
    {
        ...
        public async Task<ActionResult<IEnumerable<Estado>>> Get(int id){
            var item = await _UnitOfWork.Estados.GetIdAsync(id);
            return Ok(item); 
        }
        ...
        public async Task<ActionResult<Estado>> Post(EstadoDto itemDto){
            var item = _mapper.Map<Estado>(itemDto);
            this._UnitOfWork.Estados.Add(item);
        ...
        public async Task<ActionResult<Estado>> Put(int id, [FromBody] Estado item){
            if (item.Id == 0){
                item.Id=id;
            }
            if (item.Id != id){
            ...
            _UnitOfWork.Estados.Update(item);
            await _UnitOfWork.SaveAsync();
            return item;
        }
        [HttpDelete("{id}")]
        ...
        public async Task<ActionResult> Delete(int id){
            ...
            _UnitOfWork.Estados.Remove(item);
            await _UnitOfWork.SaveAsync();
            return NoContent();
        }
    }
